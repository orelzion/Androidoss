...
הוא מאפשר לנו ליצור קלאסים ״מנהלים״ שהם מקור אמת אחד, אמין ונגיש מכל קלאס ומכל ת׳רד. והוא שימושי במיוחד באנדרואיד, כסביבה מרובת תהליכים. הוא כל כך נפוץ ואלמנטרי ולכן כמובן הוא גם מקור לא אכזב של קרבות מפתחים, איך בדיוק לכתוב סינגלטון ומתי.
...
באובג׳קט אוריינטד אנחנו יוצרים אובייקטים, יש לנו קלאס שמגדיר לנו מה האובייקט יכיל, אנחנו מאתחלים את האובייקט ומשתמשים בו מאותו רגע. אם קלאס מסויים מכיל אובייקט מסויים אי אפשר להשתמש באותו אובייקט בשני הקלאסים.
...
- יש לנו קלאס שאמור לנהל לנו אחסון של פירות. למחסן מותר להכניס רק פירות שעוד לא קיימים בו.
...
{{< highlight java "linenos=table,hl_lines=3 5-8 17,linenostart=1" >}}
class FruitItemFragment extends Fragment {
  //... Init
	private FruitStorageManager fruitStorageManager;
  ...
  private Fruit getFruitToDisplay() {
  	fruitStorageManager.getFruitByProperty(mFruitColor, mFruitName);  
  }
}
{{< / highlight >}}

גם לכאן הוספנו משנה מסוג `FruitStorageManager` גם פה אנחנו מאתחלים אותו וקוראים לו במקום בו נרצה להשתמש בו.

**אז איפה הבעיה?** אני מקווה ששמתם לב שבעצם יש לנו שני אינסטנסים (Instance = מופע, או אובייקט שנוצר מקלאס) שונים. זאת אומרת שאם אני מוסיף פירות חדשים בפרגמנט `FruitItemFragment` האינסטנס של `FruitStorageManager` שבפרגמנט `FruitListFragment` לא יהיה מודע לזה כלל, כי למעשה אלו שתי אובייקטים שונים לחלוטין.

זה ממש לא מה שרצינו, נכון?

אנחנו רוצים מקום אחד שמנהל לנו את כל האחסון. אנחנו רוצים **סינגלטון**.
...
למרות ששתי ההגדרות האלה נשמעות פשוטות, למעשה כשצוללים לתוכן פנימה מגלים שלא תמיד כל כך לשמור עליהן.
...
זה די קל, אנחנו פשוט נהפוך את הקונסטרקטור שלנו ל `private` כך לקלאסים חיצוניים לא תהיה גישה אליו וממילא לא תהיה אפשרות ליצור אותו.
...
**פונקציה סטטית** היא פונקציה מיוחדת, היא אמנם חלק מהקלאס אבל היא זמינה ברמת **האפליקציה**. ובשונה מפונקציות וממשתנים רגילים שחיים ברמת האובייקט (הם **לא** משותפים לכל האינסטנסים) פונקציות ומשתנים סטטיים חיים ברמת הקלאס (הם **כן** משותפים לכל האינסטנסים).
...
מתוך הפרגמנטים שלנו, אנחנו נקרא לסינגלטון ישירות, כבר לא נצטרך להחזיר אינסטס של המחסן. זה ייראה ככה
...
כאן הסרנו למעשה את פעולת האיתחול של הסינגלטון מהגדרת המשתנה. הסינגלטון שלנו יווצר רק אם כשמישהו קורא לו הוא עדיין לא נוצר.

### Thread Safey (מקביליות)
...
נדמיין שת׳רד מסויים - נקרא לו אבי - קורא לסינגלטון שלנו. הוא נכנס ל`getInstance` בודק ורואה שהוא `null` ואז כמובן הולך ויוצר אותו. **במקביל** ת׳רד אחר - נקרא לו יוסי - עושה בדיוק את אותו הדבר. הוא נכנס ל`getInstance` בודק ורואה שהוא `null` (כי אנחנו חיים בעולם מקביל נכון? האינסטנס שאבי יוצר עוד לא נוצר) ויוצר אחד חדש! כאשר שניהם יסיימו יש לנו כאן שני אינסטנסים שנוצרו (כשאחד ידרוס את השני), ולמעשה כבר אין לנו ממש **סינגלטון**. באסה.

אבל לא באנו לאן כדי להתבאס, באנו לכאן כדי לפתור דברים. 

למזלנו המפתחים של ג׳אווה כבר חשבו על זה בשבילנו, וכדי לפתור את המצבים האלה (שמכונים race-condition) הם יצרו את המילה השמורה **synchronized**. 
...
אבל, גם איבדנו את המקביליות. כמה פעמים כבר נגיע לסיטואציה של יצירת אינסטנס חדש? פעם אחת בלבד. ובשביל הפעם האחת הזאת אנחנו מבטלים את המקביליות של `getInstance` לחלוטין? זה לא הגיוני.
...
במקום להפוך את כל הפונקציה למסונכרת, אנחנו ניצור בלוק מסונכרן רק אם `sInstance` הוא `null`.
...
הסרנו את הסנכרון מהפונקציה והעברנו אותה לבלוק. עכשיו אבי מגיע, רואה ש`sInstance` הוא `null` ונכנס לבלוק המסונכרן. מכיון שאין שם עוד אף אחד אחר הוא יוצר את האינסטנס ויוצא. יוסי שהגיע בזמן שאבי יוצר את האינסטנס גם רואה ש`sInstance` הוא `null` אז הוא מנסה להגיע לבלוק ונחסם, אחרי שהחסימה שלו תשתחרר (אבי עזב את הבלוק), הוא יכנס אל הבלוק אבל אז (מכיון שהוספנו גם לשם בדיקת null) הוא יגלה ש `sInstance` כבר לא `null` וימשיך בלי ליצור כלום.

מושלם! יש לנו גם לייזי לודינג, גם מקביליות ונשארנו סינגלטון.

## סינגלטון בקוטלין

קוטלין הציגה לעולם את השימוש ב [object](https://kotlinlang.org/docs/tutorials/kotlin-for-py/objects-and-companion-objects.html). מה זה object אתם שואלים? ובכן סינגלטון. כן כן, המפתחים של קוטלין יודעים כמה הרבה אנחנו משתמשים בסינגלטון, הם מודעים לכל הבעיות שהצגנו כאן בפוסט והם החליטו לאפשר לנו לבצע את זה בקלות מרשימה.
